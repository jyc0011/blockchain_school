...
class Blockchain:
    ...
    def chain_is_valid(self, chain):
            ...
            for i in range(1, len(chain)):
                ...
                # 채우시오 : 현재 체인 i를 이용해서 채우시오
                prev_block = chain[i-1]
                #채우시오 : 이전 노드 i를 이용해서 채우시오
                ...
                print(block_hash)
                if block_hash[:4] !='0000' and i != 1:
                #채우시오 : i가 1이 아니거나 hash값 찾은 hash값 조건 넣기
                    validChain = False

                if current_block['previous_block_hash'] != prev_block['hash'] and i != 1:
                    #채우시오 : i가 1이 아니거나 현재 블럭의 previous_hash값과 이전 블럭의 hash값이 같은지 비교
                    validChain = False
    ...
    def create_merkle_tree(self, transactions):
        if len(transactions) == 0:
            ...
            # 제네시스생성 및 tx가 처음부터 한개일때 (coinbase tx만 있을때)
            #채우시오 : 가장 마지막 tx를 trasactions에 추가
            new_level = []
            for i in range(0, len(transactions), 2):
            #채우시오, 채우시오, 채우시오): 0부터 len(tx)까지
                left = transactions[i]
